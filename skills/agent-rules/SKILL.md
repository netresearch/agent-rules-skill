...
This skill generates the AGENTS.md file. Two related skills cover adjacent concerns:

- **`agent-harness-skill`** — verifies and bootstraps the broader agent-readiness harness (AGENTS.md as index, `docs/` structure, CI verification, enforcement mechanisms). Use it when making any repo agent-ready end-to-end; it invokes this skill when AGENTS.md is missing or stale.
- **`skill-repo-skill`** — defines the structure of a *skill* repository (plugin.json, `skills/<name>/SKILL.md`, split licensing, release workflows). The resulting repo should also have an AGENTS.md generated by this skill.
